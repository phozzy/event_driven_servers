 ...
 * All rights reserved.
 *
 * $Id: conf.c,v 1.20 2015/03/14 06:11:31 marc Exp marc $
 *
 */

#include "headers.h"

static const char rcsid[] __attribute__ ((used)) = "$Id: conf.c,v 1.20 2015/03/14 06:11:31 marc Exp marc $";
...
void parse_decls(struct sym *sym)
{
	...
	case S_tls:
		...
		continue;
	    default:
		parse_error_expect(sym, S_certfile, S_keyfile, S_passphrase, S_unknown);
	    }
#endif
	case S_rebalance:
	    sym_get(sym);
