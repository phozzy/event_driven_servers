...
static int cache_lookup(mavis_ctx * mcx, av_ctx * ac)
{
	...
	for (i = 0; i < AVPC_TABLE_SIZE; i++) {
		...
		logmsg("STAT %s: %s: Q=%llu C=%llu T=%ld "
		       "q=%llu c=%llu t=%ld #=%u",
		       MAVIS_name,
		       mcx->cache[i].type,
		       mcx->cache[i].counter_query,
		       mcx->cache[i].counter_cached,
		       (long) io_now.tv_sec - mcx->startup_time,
		       mcx->cache[i].counter_p_query, mcx->cache[i].counter_p_cached, (long) io_now.tv_sec - mcx->lastdump, mcx->cache[i].count);
