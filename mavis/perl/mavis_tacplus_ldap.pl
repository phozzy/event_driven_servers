#!/usr/bin/env perl
# $Id: mavis_tacplus_ldap.pl,v 1.53 2016/10/30 07:31:44 marc Exp marc $
#
...
while ($in = <>) {
	...
	if ($authdn){
		...
		$mesg = $ldap->search(base => $authdn, scope=>'base', filter=>'(objectclass=*)', attrs=>['shadowExpire','memberOf']);
		if ($mesg->code){
			$V[AV_A_USER_RESPONSE] = $mesg->error . " (" . __LINE__ . ")";
			goto fatal;
		}
