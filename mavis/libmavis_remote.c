...
static int mavis_send_in(mavis_ctx * mcx, av_ctx ** ac)
{
	...
	logmsg
	    ("STAT %s:  O=%llu I=%llu B=%lu T=%lu "
	     "o=%llu i=%llu b=%lu t=%lu",
	     MAVIS_name, count_s, count_r,
	     backlog_max, (u_long) io_now.tv_sec - mcx->startup_time, count_s_p, count_r_p, backlog_max_p, (u_long) io_now.tv_sec - mcx->lastdump);
